...
class ActivityRepository {
	...
	getMilesWalked(idNum, date, strideLength) {
		const activityOnDay = this.activityData.find(
			(day) => day.date === date && day.userID === idNum
		);
		return parseFloat(
			((activityOnDay.numSteps * strideLength) / 5280).toFixed(2)
		);
	}

	getMinutesActivePerDay(idNum, date) {
		const activityDay = this.activityData.find(
			(day) => day.userID === idNum && day.date === date
		);
	...
	getUsersActivityDataPerWeek(id, dateParam, property) {
		const userData = this.getActivityDataForUser(id);
		const endDateIndex = userData.findIndex((date) => {
			return date.date === dateParam;
		});
		const weeklyRange = userData.slice(endDateIndex - 6, endDateIndex + 1);
		const weeklyPropertyActivity = weeklyRange.map((date) => {
			return { date: date.date, [property]: date[property] };
		});
		return weeklyPropertyActivity;
	}

	assessUserStepGoalCompletionPerDay(idNum, date, stepGoal) {
		const userActivityDay = this.activityData.find(
			(day) => day.userID === idNum && day.date === date
		);
	...
	assessUserStepGoalCompletionAllTime(idNum, stepGoal) {
		const userActiveDays = this.activityData.filter(
			(day) => day.userID === idNum && day.numSteps >= stepGoal
		);
	...
	getUserAverage(idNum, property) {
		...
		const userAverage =
			userActivity.reduce((counter, currentDay) => {
	...
	getAllUsersAverage(date, property) {
		...
		const allUsersAverage =
			activityDate.reduce((acc, cur) => {
				acc += cur[property];
				return acc;
